  ...
  try {
    ...
    if (booking && (status === 'approved' || status === 'declined')) {
      ...
      if (status === 'approved') {
        ...
        // Fan card approval → upgrade user plan for 1 year
        if (booking.bookingType === 'fan_card' || booking.bookingType === 'fan_card_platinum') {
          const plan = booking.bookingType === 'fan_card_platinum' ? 'platinum' : 'premium';
          const expiresAt = new Date(Date.now() + 365 * 24 * 60 * 60 * 1000);
          await q('UPDATE users SET plan=$1, plan_expires_at=$2 WHERE id=$3', [plan, expiresAt, booking.userId]);
          sendPlanChanged({ name: booking.userName, email: booking.userEmail, plan, planExpiresAt: expiresAt }).catch(() => {});
        }
      }
    }
  } catch { res.status(500).json({ error: 'Database error' }); }
...
app.patch('/api/admin/users/:id/membership', authenticate, adminOnly, async (req, res) => {
  try {
    ...
    // Set user plan + 1-year expiry
    const plan = tier === 'platinum' ? 'platinum' : 'premium';
    const expiresAt = new Date(Date.now() + 365 * 24 * 60 * 60 * 1000);
    await q('UPDATE users SET plan=$1, plan_expires_at=$2 WHERE id=$3', [plan, expiresAt, req.params.id]);
    res.json({ success: true });
    ...
    sendPlanChanged({ name: targetUser.name, email: targetUser.email, plan, planExpiresAt: expiresAt }).catch(() => {});
  } catch(e) { res.status(500).json({ error: e.message }); }
});
